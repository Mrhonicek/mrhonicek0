...
class _MapPageState extends State<MapPage> {
  ...
  String _currentMapStyle = MapboxStyles.STANDARD_EXPERIMENTAL;
  final List<String> _mapStyles = [
    MapboxStyles.STANDARD_EXPERIMENTAL,
...
// Mapping styles to more readable names
  final Map<String, String> styleNames = {
    MapboxStyles.STANDARD_EXPERIMENTAL: "Standard",
    MapboxStyles.SATELLITE: "Satellite",
    MapboxStyles.OUTDOORS: "Outdoors",
    MapboxStyles.LIGHT: "Light",
...
// ✅ Function to get report count from Supabase
  ...
  Future<void> _refreshMap() async {
    try {
      ...
      await mapboxMapController?.loadStyleURI(_currentMapStyle);
  ...
  void _toggleMapStyle(String selectedStyle) {
    setState(() {
      ...
      mapboxMapController?.loadStyleURI(_currentMapStyle).then((_) {
        onMapStyleLoaded(
            mapboxMapController); // Restore the polygon after style changes
      });
    });
  }
  ...
  Future<void> _initializeRealtimeListener({int retryCount = 0}) async {
    try {
      ...
      if (retryCount < 3 && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Could not establish live updates connection'),
            duration: Duration(seconds: 3),
          ),
        );
      }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _refreshMap, // Add this function call
            tooltip: 'Refresh Map',
          ),
        ],
      ),
        ...
        children: [
          ...
          Positioned.fill(
            child: MapWidget(
              ...
              styleUri: _currentMapStyle, // Set initial style
              textureView: true,
