...
const Duration FLOOD_ALERT_INTERVAL = Duration(minutes: 10);
...
Timer? floodAlertTimer; // Add this new timer
DateTime? lastFloodAlertTime; // Track when we last showed an alert
...
Future<void> _storeEmergencyAlert(String title, String message,
  ...
  try {
    ...
    final alertData = {
      ...
      'navigate': data['navigate'] ?? 'newsfeed',
      'shown': false,
    };
...
Future<void> setupFloodMonitoring() async {
  try {
    ...
    // Set up periodic flood alert timer (every 10 minutes)
    _setupFloodAlertTimer();
...
void _setupFloodAlertTimer() {
  floodAlertTimer = Timer.periodic(FLOOD_ALERT_INTERVAL, (timer) {
    print("⏰ Periodic flood check triggered");
    if (currentFloodReportCount >= 5) {
      _triggerPeriodicFloodAlert(currentFloodReportCount);
    }
  });
  print(
      "⏰ Flood alert timer set for every ${FLOOD_ALERT_INTERVAL.inMinutes} minutes");
}
...
void _handleFloodReportsUpdate(List<Map<String, dynamic>> reports) {
  try {
    ...
    // Trigger immediate alert if threshold reached and we haven't alerted recently
    if (reportCount >= 5) {
      final now = DateTime.now();
      final shouldShowAlert = lastFloodAlertTime == null ||
          now.difference(lastFloodAlertTime!).inMinutes >= 10;
...
void _triggerPeriodicFloodAlert(int reportCount) {
  final now = DateTime.now();

  // Only show if we haven't shown an alert in the last 10 minutes
  if (lastFloodAlertTime == null ||
      now.difference(lastFloodAlertTime!).inMinutes >= 10) {
    print("⏰ Triggering periodic flood alert for $reportCount reports");
    _triggerFloodAlert(reportCount);
    lastFloodAlertTime = now;
  }
}
...
void _triggerFloodAlert(int reportCount) {
  ...
  // GLOBAL ALERT: Show full screen alert regardless of current page
  _showGlobalFloodAlert(
      title, message, reportCount >= 6 ? 'critical' : 'warning');
}

void _showGlobalFloodAlert(String title, String message, String warningGauge) {
  // This function will work from any page in the app
  final context = MyApp.navigatorKey.currentContext;

  if (context != null) {
    // If app is active, show full screen alert immediately
    print("📱 App is active - showing immediate flood alert");
    _showFullScreenAlert(title, message, warningGauge);
  } else {
    // If app is not active, store for when it becomes active
    print("📱 App not active - storing flood alert for later");
    pendingAlertData = {
      'title': title,
      'message': message,
      'warning_gauge_lvl': warningGauge,
...
Future<void> cleanupFloodMonitoring() async {
  ...
  floodAlertTimer?.cancel(); // Add this line
  floodAlertTimer = null; // Add this line
  ...
  try {
    ...
    if (alertDataString != null) {
      ...
      if (!hasShown && MyApp.navigatorKey.currentContext != null) {
        ...
        // Navigate if needed
        final navigateTo = alertData['navigate'] as String? ?? 'newsfeed';
...
class _MyAppState extends State<MyApp> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    if (state == AppLifecycleState.resumed) {
      ...
      // Also check current flood situation
      Future.delayed(const Duration(milliseconds: 1500), () {
        if (currentFloodReportCount >= 5) {
          ...
          if (shouldShowAlert) {
            _triggerFloodAlert(currentFloodReportCount);
          }
        }
      });
    } else if (state == AppLifecycleState.paused) {
      // App is going to background, make sure monitoring continues
      print("📱 App paused, flood monitoring continues in background");
    }
  }
