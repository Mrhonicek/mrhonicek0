...
import 'package:gismultiinstancetestingenvironment/pages/faqpage.dart';
...
class _NewsFeedState extends State<NewsFeed> {
  ...
  // ADD THESE VARIABLES FOR TEMPORARY BAN COUNTDOWN
  Timer? _tempBanTimer;
  ...
  void initState() {
    ...
    _isMounted = true;
    _checkConnectivity();
    _setupRealtimeAccessMonitoring();
  ...
  Future<void> _setupRealtimeAccessMonitoring() async {
    ...
    try {
      // Create realtime channel for the current user's profile
      _accessChannel = Supabase.instance.client
          .channel('profile_access_${user.id}')
          .onPostgresChanges(
            ...
            callback: (payload) {
              print('🔄 Profile update detected: ${payload.newRecord}');

              // MODIFY THIS - Check for both permanent and temporary bans
              final newRecord = payload.newRecord;
              if (newRecord != null) {
                final canAccess = newRecord['canAccess'];
                if (canAccess == 0) {
                  print('🚫 User permanently banned - forcing logout');
                  if (_isMounted) {
                    _showAccountDisabledDialog();
                  }
                } else if (canAccess == 2) {
                  print('⏰ User temporarily banned - forcing logout');
                  if (_isMounted) {
                    final tempBanDate = newRecord['temp_ban_date'];
                    _showTemporaryBanDialog(tempBanDate);
                  }
                }
              }
            },
      ...
      print('✅ Realtime access monitoring started for user: ${user.id}');
    } catch (e) {
      print('❌ Error setting up realtime access monitoring: $e');
      // Fallback to periodic check if realtime fails
      _setupPeriodicAccessCheck();
    }
  }

  void _setupPeriodicAccessCheck() {
...
// MODIFY THIS METHOD - Check if user account is still accessible
  Future<void> _checkUserAccess() async {
    final user = Supabase.instance.client.auth.currentUser;
    if (user == null) return;

    // Skip check for anonymous users (no email)
    if (user.email == null || user.email!.isEmpty) return;

    try {
      final profile = await Supabase.instance.client
          .from('profiles')
          .select('canAccess, temp_ban_date')
          .eq('id', user.id)
          .single();

      if (profile != null) {
        final canAccess = profile['canAccess'];
        ...
        if (canAccess == 0) {
          ...
          // Temporary ban
          final tempBanDate = profile['temp_ban_date'];
          _showTemporaryBanDialog(tempBanDate);
        }
      }
    } catch (e) {
      print('❌ Error checking user access: $e');
      // Don't log out on error - could be network issue
    }
  }

// ✅ Show account disabled dialog and force logout (for permanent ban)
        ...
        return AlertDialog(
          title: Row(
            children: [
              ...
              SizedBox(width: 8),
              Text('Account Permanently Disabled'),
            ],
          ),
          content: Text(
            'Your account has been permanently disabled by an administrator due to repeated violations. You will be automatically logged out.',
          ),
          actions: [
            TextButton(
              onPressed: () {
  ...
  // ADD THIS METHOD - Show temporary ban dialog with countdown
  void _showTemporaryBanDialog(String? tempBanDateStr) {
    if (tempBanDateStr == null) {
      _showAccountDisabledDialog(); // Fallback to permanent ban dialog
      return;
    }

    try {
      final tempBanDate = DateTime.parse(tempBanDateStr);
      final banEndTime = tempBanDate.add(Duration(hours: 24));
      final now = DateTime.now();

      if (now.isAfter(banEndTime)) {
        // Ban has expired, should not reach here but handle gracefully
        print('⚠️ Ban has expired but canAccess is still 2');
        return;
      }

      final remainingDuration = banEndTime.difference(now);
        ...
        builder: (BuildContext context) {
          return StatefulBuilder(
            builder: (context, setState) {
              ...
              _tempBanTimer?.cancel();
              _remainingTime = remainingDuration;

              _tempBanTimer = Timer.periodic(Duration(seconds: 1), (timer) {
                if (_remainingTime != null && _remainingTime!.inSeconds > 0) {
                  ...
                  timer.cancel();
                  // Ban expired, close dialog and refresh
                  Navigator.of(context).pop();
                  _refreshPosts();
                }
              });
              ...
              return AlertDialog(
                ...
                content: Column(
                  ...
                  children: [
                    Text(
                      'Your have been temporarily restricted for reporting false information. Contact an administrator for more details.',
                    ...
                    SizedBox(height: 8),
                    Text('Ban Date: ${_formatDateTime(tempBanDate)}'),
                    Text('Ban Ends: ${_formatDateTime(banEndTime)}'),
                    SizedBox(height: 12),
                    Container(
                    ...
                    SizedBox(height: 12),
                    Text(
                      'You will be automatically logged out. Please wait for the restriction period to end before accessing the app again.',
                      style: TextStyle(
                        fontSize: 14,
      ...
      print('❌ Error parsing temp ban date: $e');
      _showAccountDisabledDialog(); // Fallback
    }
  }

  // ADD THIS METHOD - Format date and time
  String _formatDateTime(DateTime dateTime) {
    return '${dateTime.day}/${dateTime.month}/${dateTime.year} ${dateTime.hour.toString().padLeft(2, '0')}:${dateTime.minute.toString().padLeft(2, '0')}';
  }

  // ADD THIS METHOD - Format remaining time as HH:MM:SS
  ...
  Future<void> _forceSignOut() async {
    // MODIFY THIS - Clean up timer before signing out
    _tempBanTimer?.cancel();
  ...
  void dispose() {
    ...
    // MODIFY THIS - Clean up realtime subscription and timer
    _accessChannel?.unsubscribe();
